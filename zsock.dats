...
implement zsock_test (verbose) = () where {
    ...
    val format = $extfcall (zstr_t, "zsys_sprintf", "tcp://127.0.0.1:%d", 5560)
	val rc = zsock_unbind (writer, zstr_to_string format)
	val _ = assert (rc = 0)
    val _ = zstr_free format

	val _ = zclock_sleep 100u

    val format = $extfcall (zstr_t, "zsys_sprintf", "tcp://127.0.0.1:%d", 5560)
	val rc = zsock_bind (writer, "tcp://127.0.0.1:5560")
    ...
    #define DYNAMIC_FIRST 49152
    #define DYNAMIC_LAST 65535
    ...
    val rc = zsock_bind (writer, "inproc://test.writer")
    val _ = assert (rc = 0)
    ...
    val rc = zsock_signal (writer, i2byte 123)
    val _ = assert (rc = 0)
    val rc = zsock_wait reader
    val _ = assert (rc = 123)

    (* can't test zsock send/recv because of vararg *)
